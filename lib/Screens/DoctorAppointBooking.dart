...
import 'package:meddoc/Widgets/LoadingContainer.dart';import '../Widgets/AppointmentCalender.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      children: [
        ...
        loading?LoadingContainer():Expanded(
            child: ListView(
                shrinkWrap: true,
                children: [
                ...
                Gap(10),
                appointPossible?AppointmentCalender(DoctorUID: widget.DoctorUID):Column(
                  children: [
                    Padding(padding: EdgeInsets.all(16),
                        child: Column(
                          children: [
                            Text("You can't book another booking if the appointment is not done or approved by the specialist",style: GoogleFonts.montserrat(fontSize: 16,fontWeight: FontWeight.w400),textAlign: TextAlign.center,),
                            Gap(20),
                            InkWell(child: SmallPrimaryButton(title: 'See Appointments',),onTap: (){
                              Navigator.push(context, MaterialPageRoute(builder: (builder)=>PatientNavigationBar(selectedindex: 3,)));
                            },)
                          ],
                        )
                    ),
    ...
    event.listen((event) { 
     if(event.snapshot.exists){
       for(DataSnapshot dataSnapshot in event.snapshot.children){
         if(dataSnapshot.child("patientID").value.toString()==FirebaseAuth.instance.currentUser!.uid){
