...
class CompleteDoctorProfile extends StatefulWidget {
  ...
  String firstName;
  String surName;
  CompleteDoctorProfile( {Key? key, required this.imageURL,required this.name,required this.email,required this.age,required this.password,required this.gender,required this.type,required this.firstName,required this.surName}) : super(key: key);
...
class _CompleteDoctorProfileState extends State<CompleteDoctorProfile> {
                  ...
                  Column(
                    children: [
                      ...
                      _loading?Center(child: CircularProgressIndicator()):GestureDetector(
                          child: PrimaryButton(
                              width: size.width, height: 50, title: "Finish"),
                          onTap: () {
                            processData();
  ...
  void addtoDatabase(String uid,String downloadURL) {
   ...
   DatabaseReference ref= FirebaseDatabase.instance.ref("Users").child("Doctors").child(uid);
