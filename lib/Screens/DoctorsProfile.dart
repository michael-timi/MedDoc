...
class DoctorsProfile extends StatefulWidget {
  String docID;
  bool doctor;
   DoctorsProfile({Key? key,required this.docID,required this.doctor}) : super(key: key);
...
class _DoctorsProfileState extends State<DoctorsProfile> {
                    ...
                    children: [
                      Container(
                        decoration:BoxDecoration(
                          ...
                          child: ProfilePictureContainer(name: name,imageAsset: imgURL,age: age,clinicAddress: hospitalAddress,phonenumber: phoneNumber,)),
                      Container(
                        decoration: BoxDecoration(color: Styles.secondryColor),
                       child:StatContainer(heading1: "Speciality",description1: speciality,heading2: "Experience",description2: experience+" Years",heading3: "Gender",description3: gender,),
                      ),

                      ButtonsContainer(doctor:widget.doctor,docID:widget.docID,name:name),
                      MainReviewsContainer(doc: widget.doctor,docID: widget.docID,),
  ...
  void getDoctorInfo() {
    ...
    DatabaseReference ref=FirebaseDatabase.instance.ref("Users").child("Doctors").child(widget.docID);
    ...
    event.listen((event) {
      if(event.snapshot.exists){
        setState(() {
          ...
          hospitalAddress=event.snapshot.child("hospitalAddress").value.toString();
        });
      }
